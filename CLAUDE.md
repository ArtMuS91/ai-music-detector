...
- `src/` — .NET 10 solution (`AiMusicDetector.slnx`)
  ...
  - `Infrastructure` — implements `Core.Repositories` (EF Core/Postgres) and the external-integration half of `Core.Services` (yt-dlp), under matching `Repositories`/`Services` folders; references `Core`
  - `Analysis` — implements the business-logic half of `Core.Services` (`IAnalysisService` today; detection/aggregation later) under its own `Services` folder; references `Core`
...
### Prerequisites
- Docker is the only prerequisite for running the API: `docker compose up -d` from the repo root builds and starts both Postgres and the API (with yt-dlp bundled in its image), migrating the database on startup. The API is reachable at `http://localhost:5214`, same as the `dotnet run` dev workflow.
- Running the API with `dotnet run` instead of Docker needs two things Docker otherwise provides: Postgres reachable at the `Postgres` connection string in `appsettings.json` (`docker compose up -d postgres` is enough), and `yt-dlp` on the host `PATH` (override the location with `YtDlp:ExecutablePath` if it lives elsewhere).

### API (`src/Api`, run from repo root)
- Build: `dotnet build`
- Run (dev, hot-reload, needs local Postgres + yt-dlp — see Prerequisites): `dotnet run --project src/Api` — serves on `http://localhost:5214` (see `src/Api/Properties/launchSettings.json`); in Development only, the OpenAPI document is served at `/openapi/v1.json` and the Scalar UI over it at `/scalar/v1`.
...
- **Layering rule: endpoints never depend on a repository directly.** `Api` endpoints call a `Core.Services` interface (e.g. `IAnalysisService`) that owns the business logic (validation, orchestration) and is the only thing that talks to `Core.Repositories`. This keeps request-time rules in one place instead of scattered across endpoint handlers. `AnalysisService` (in `Analysis/Services`) is the current example — it validates the URL and delegates to `IAnalysisJobRepository`. Background services (e.g. `AcquisitionWorker`) are the pipeline's own business logic, not a thin API layer, so they may use repositories directly.
...
- Work is queued, not done in the request: `POST /api/analyze` persists a job and returns 202, `AcquisitionWorker` claims it, and the client polls `GET /api/analyze/{id}`. Claiming uses `FOR UPDATE SKIP LOCKED` so more than one worker can poll the same queue safely.
